...
def Img(keyword,max,directory): #메인
    if(keyword == ''):
    ...
    image_search(keyword,max,folder_directory)
    
def image_search(keyword,max,folder_directory):
    ...
    if(len(failed) == 1):
        print("total {} successfully downloaded".format(max))
    else:
